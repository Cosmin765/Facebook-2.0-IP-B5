...
public interface AdCompanyRepository extends JpaRepository<AdCompany, Integer> {
    @Query(value = "SELECT SUM(click_count) FROM ad_clicks where ad_id IN (SELECT id FROM ads WHERE ad_company_id = (SELECT id FROM ad_companies WHERE id = :id))", nativeQuery = true)
